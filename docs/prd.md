...
- **注意**：DOSBox-X 的目标用户群体年龄偏大，视力条件较差，因此界面需要足够大、清晰、不易疲劳。所有 UI 元素（图标、字体、控件尺寸）均需按 **2 倍**放大设计。
...
> - `docs/specs/dosbox-x.reference.conf` — 常用配置项及说明。
> - `docs/specs/dosbox-x.reference.full.conf` — 包含所有高级选项的完整参考。
>
...
- **无障碍与易读性**：为照顾年长用户群体的视力需求，界面所有元素均按 2x 放大设计：
  - 全局应用字体设为 13pt（约为系统默认的 1.5-2 倍）。
  - 工具栏图标显示尺寸为 32×32（原始 GIF 资源为 16×16，加载时平滑缩放 2 倍）。
  - 封面缩略图 64×80（原始设计 32×40 的 2 倍）。
  - 封面详情图 320×400（原始设计 160×200 的 2 倍）。
  - 主窗口最小尺寸 1200×800。
...
- **格式**：GIF，16×16 像素。
- **显示尺寸**：所有图标在应用中统一以 **32×32 像素**显示（加载时通过 `SmoothTransformation` 平滑缩放 2 倍），以适配年长用户的视力需求。图标显示尺寸由 `resources.py` 中的 `_ICON_DISPLAY_SIZE` 常量统一控制。
...
| 默认封面占位图 | `assets/default_cover.png` | PNG，160×200（详情面板中以 320×400 显示） | 游戏未指定封面图时的占位图。设计为灰色底色 + 居中的游戏手柄或光碟剪影 + "No Cover" 文字提示，保持视觉整洁 |
